...
const CommunicationPage = () => {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            ...
            const info = type === 'lawyer' ? lawyerInfo : clientInfo;
            setCurrentUser(info);
        }
    ...
    const handleSendMessage = async (e) => {
        ...
        try {
            const data = await authService.sendMessage({
                ...
                receiverModel: selectedContact.role, // Changed from receiverType to match Next.js API
                content: newMessage
            });
                            ...
                            <div className="messages-container">
                                {messages.length === 0 ? (
                                    ...
                                    messages.map((msg, index) => (
                                        <div
                                            key={index}
                                            className={`message-row ${msg.senderId === (currentUser?._id || currentUser?.id || currentUser?.lawyerId) ? 'sent' : 'received'}`}
                                        >
