    ...
    useEffect(() => {
        const fetchCases = async () => {
            try {
                ...
                const email = clientInfo.email || 'client@example.com';
                const data = await authService.getClientCases(email);
                setCases(data.cases || []);
            } catch (error) {
    ...
    return (
        <div className="mycases-body">
            <div className="mycases-container">
                <Link to="/client-portal" className="back-link">← Back to Portal</Link>

                <header className="mycases-header">
                    <h1>📋 My Cases</h1>
                    <p>Track and manage your filed cases</p>
                    <div className="mycases-header-actions">
                        <Link to="/e-filing" className="btn-mycases btn-mycases-primary">+ File New Case</Link>
                        <Link to="/client-dashboard" className="btn-mycases btn-mycases-secondary">Dashboard</Link>
                    </div>
                </header>
                ...
                {loading ? (
                    <div style={{ textAlign: 'center', padding: '50px' }}>Loading cases...</div>
                ) : filteredCases.length === 0 ? (
                    <div className="empty-state" style={{ textAlign: 'center', background: 'white', padding: '60px', borderRadius: '16px' }}>
                        <div style={{ fontSize: '4rem', marginBottom: '20px' }}>📂</div>
                        <h3>No Cases Found</h3>
                        <p>You haven't filed any cases yet matching these criteria.</p>
                        <Link to="/e-filing" className="btn-mycases btn-mycases-primary" style={{ marginTop: '20px', display: 'inline-block', background: '#c19651', color: 'white' }}>File Your First Case</Link>
                    </div>
                ) : (
                                ...
                                <div className="mycase-card-body">
                                    ...
                                    <div className="mycase-progress-section">
                                        <div className="progress-header">
                                            <span style={{ fontWeight: '700', fontSize: '0.9rem' }}>Case Progress</span>
                                            <span style={{ color: '#64748b', fontSize: '0.8rem' }}>Stage {c.currentStage || 1} of 5</span>
                                        </div>
                                        <div className="mycase-progress-bar">
                                            <div className="mycase-progress-fill" style={{ width: `${((c.currentStage || 1) / 5) * 100}%` }}></div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="doc-list" style={{ display: 'grid', gap: '10px' }}>
                                        {selectedCase.documents.map((doc, idx) => (
                                            <div key={idx} className="doc-item" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 15px', background: '#f8fafc', borderRadius: '10px', border: '1px solid #e2e8f0' }}>
                                                ...
                                                <a href={`http://localhost:5000/api/cases/${selectedCase.id}/documents/${doc.filename}`} target="_blank" rel="noopener noreferrer" style={{ background: '#c19651', color: 'white', padding: '6px 12px', borderRadius: '6px', textDecoration: 'none', fontSize: '0.85rem', fontWeight: '700' }}>Download</a>
                                            </div>
                                        ))}
                                    </div>
                                </div>
