...
const LawyerAIDraftingPage = () => {
    ...
    const handleReview = async () => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5001/api/drafting/${currentDraft._id}/review`, {
                method: 'POST',
                headers: {
    ...
    const exportToWord = async () => {
        ...
        try {
            const token = localStorage.getItem('lawyerToken') || sessionStorage.getItem('lawyerToken');
            const response = await fetch(`http://localhost:5001/api/drafting/export/${currentDraft._id}/docx`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
                            ...
                            {!currentDraft && !loading ? (
                                ...
                                <div
                                    ...
                                    dangerouslySetInnerHTML={{
                                        __html: parseMarkdown(displayedContent) + (displayedContent.length < (currentDraft?.content?.length || 0) ? '<span className="cursor-blink">▊</span>' : '')
                                    }}
                                />
                            )}
                        </div>
