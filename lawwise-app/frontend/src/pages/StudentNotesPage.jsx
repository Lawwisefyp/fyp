...
const StudentNotesPage = () => {
    ...
    const [activeFolder, setActiveFolder] = useState(null); // 'all' | 'public' | folderId
    ...
    const fetchNotes = async () => {
        ...
        try {
            ...
            if (activeTab === 'community') {
                result = await authService.fetchPublicNotes();
                if (result.success) setNotes(result.notes);
            } else {
                // Library Tab - filter by folder if active
    ...
    const getShareUrl = (note) => {
        const appUrl = `http://localhost:5173/student-library`;
    ...
    return (
        <div className="notes-page-container">
            {/* Sidebar for Library Organization */}
            <div className="notes-sidebar">
                <div className="sidebar-section">
                    <button className="back-btn" onClick={() => navigate('/student-dashboard')} style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '20px' }}>← Dashboard</button>
            ...
            <div className="notes-main-content">
                <div className="notes-header">
                    <div className="notes-title-sec">
                        <h1>{activeTab === 'community' ? 'Community Hub' : 'My Library'}</h1>
                    ...
                    {activeTab === 'library' && (
                        <button
                            ...
                            style={{ padding: '12px 25px', borderRadius: '12px', fontSize: '1rem', fontWeight: '700', border: 'none', cursor: 'pointer' }}
                        >
                            + Upload Notes
                        </button>
                    )}
                        ...
                        {notes.length > 0 ? (
                            notes.map(note => (
                                <div key={note._id} className="note-card">
                                    <div className="note-badge" style={{ backgroundColor: '#fff7ed', color: '#f57c00' }}>#{note.subject}</div>
                                    <h3>{note.title}</h3>
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', fontSize: '0.8rem', color: '#94a3b8' }}>
                                        <span>{activeTab === 'community' ? `By: ${note.uploader?.fullName}` : 'Private Note'}</span>
                                    ...
                                    <div className="note-actions">
                                        ...
                                        <div className="note-actions-row">
                                            <button onClick={() => handleDownload(note._id, note.fileUrl)} style={{ padding: '10px 14px', borderRadius: '10px', fontWeight: '600', fontSize: '0.85rem', cursor: 'pointer', background: '#f1f5f9', color: '#334155', border: '1.5px solid #94a3b8', width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>📥 Download</button>
                                            <div style={{ position: 'relative' }}>
                                                ...
                                                {openShareId === note._id && (
                                                    <div style={{ position: 'absolute', bottom: '110%', left: 0, background: 'white', borderRadius: '12px', boxShadow: '0 10px 25px rgba(0,0,0,0.15)', border: '1px solid #e2e8f0', padding: '8px', minWidth: '160px', zIndex: 100 }}>
                                                        <div className="share-opt" onClick={() => shareOnWhatsApp(note)}>WhatsApp</div>
                                                        <div className="share-opt" onClick={() => shareViaEmail(note)}>Email</div>
                                                        {activeTab === 'library' && (
                                                            <div
                                                                className="share-opt"
                                                                style={{ color: note.isPublic ? '#ef4444' : '#f57c00', fontWeight: 'bold' }}
                                        ...
                                        <button className="action-btn btn-ai" onClick={() => handleAIExplain(note._id)}>
                                            ✨ AI Explanation
                                        </button>
                                        {/* Row 3: Take Quiz */}
                                        <button className="action-btn btn-quiz" onClick={() => handleGenerateQuiz(note._id)}>
                                            🧠 Take Quiz
                                        </button>
                                    </div>
                                </div>
                ...
                <div className="modal-overlay" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
                    <div style={{ background: 'white', padding: '30px', borderRadius: '24px', width: '90%', maxWidth: '550px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px' }}>
                            <h2 style={{ margin: 0 }}>Add New Notes</h2>
                        ...
                        <form onSubmit={handleUpload}>
                            ...
                            <div style={{ marginBottom: '15px' }}>
                                <label style={{ display: 'block', marginBottom: '8px', fontWeight: '600' }}>Select Folder</label>
                                <select
                                    style={{ width: '100%', padding: '12px', borderRadius: '10px', border: '1px solid #e2e8f0' }}
                            ...
                            <div style={{ marginBottom: '20px', padding: '15px', background: '#f8fafc', borderRadius: '12px', display: 'flex', alignItems: 'center', gap: '15px' }}>
                                ...
                                />
                                <label htmlFor="isPublic" style={{ fontWeight: '600', cursor: 'pointer' }}>Share to Community Hub (Everyone can see it)</label>
                            </div>
                            <div style={{ marginBottom: '25px' }}>
                                <label style={{ display: 'block', marginBottom: '8px', fontWeight: '600' }}>File Upload (PDF/DOC)</label>
                            ...
                            <button
                                ...
                                disabled={uploading}
                                className="btn-primary"
                                style={{ width: '100%', padding: '15px', border: 'none', borderRadius: '12px', fontSize: '1.1rem', cursor: 'pointer' }}
                            >
            ...
            {showFolderModal && (
                <div className="modal-overlay" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
                    <div style={{ background: 'white', padding: '30px', borderRadius: '24px', width: '90%', maxWidth: '400px' }}>
                        <h2 style={{ marginBottom: '20px' }}>Create New Folder</h2>
                        <form onSubmit={handleCreateFolder}>
                            <input
                                style={{ width: '100%', padding: '15px', borderRadius: '12px', border: '1px solid #e2e8f0', marginBottom: '20px' }}
                                value={newFolderName}
                            ...
                            <div style={{ display: 'flex', gap: '10px' }}>
                                <button type="button" onClick={() => setShowFolderModal(false)} className="action-btn btn-secondary" style={{ flex: 1 }}>Cancel</button>
                                <button type="submit" className="action-btn btn-primary" style={{ flex: 1 }}>Create</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            {showAIModal && (
                <div className="modal-overlay" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
                    <div style={{ background: 'white', padding: '35px', borderRadius: '24px', width: '90%', maxWidth: '700px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px' }}>
                            <h2 style={{ margin: 0 }}>✨ AI Professor Explanation</h2>
                            <button onClick={() => setShowAIModal(false)} style={{ background: 'none', border: 'none', fontSize: '1.5rem', cursor: 'pointer' }}>✕</button>
                        </div>
                        <div className="ai-modal-body">
                            {explainingId ? (
                                <div style={{ textAlign: 'center', padding: '40px 0' }}>
                                    <div className="loader" style={{ fontSize: '2rem', marginBottom: '15px' }}>🤖</div>
                                    <p>AI is analyzing the legal context of your notes...</p>
                                </div>
                            ) : (
                                <div className="ai-exp-text" style={{ whiteSpace: 'pre-wrap' }}>
                                    {aiExplanation}
                                </div>
                            )}
                        </div>
                        {!explainingId && (
                            <button onClick={() => setShowAIModal(false)} className="btn-primary" style={{ width: '100%', marginTop: '30px', padding: '12px', borderRadius: '10px', border: 'none', cursor: 'pointer' }}>Got it, thanks!</button>
                        )}
                    </div>
                </div>
            )}
